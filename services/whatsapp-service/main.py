...
async def get_or_create_conversation(phone_number: str, org_id: str) -> str:
    ...
    async with async_session_factory() as session:
        ...
        if row:
            ...
            if row["updated_at"].replace(tzinfo=timezone.utc) > datetime.now(
                timezone.utc
            ) - timedelta(hours=24):
                return str(row["id"])
        ...
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            if twilio_client:
                twilio_client.messages.create(
                    body=reply_text, from_=TWILIO_WHATSAPP_NUMBER, to=phone_number
                )
            else:
...
async def send_whatsapp(req: SendMessageRequest):
    ...
    try:
        message = twilio_client.messages.create(
            body=req.body, from_=TWILIO_WHATSAPP_NUMBER, to=req.to_number
        )
